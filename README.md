...
To sort the logs I considered to use quicksort algorithm, because others sorting algorithm such as Merge Sort, heap sort, requires an extra space to sort, and the requirements talk about space efficiency. Quick sort is efficient with
the usage of divide and conquer, however there is a trade off with the pivot, if there is not efficient could have a
problem the time complexity.

Additional in the function quickSort I used parallel promises to solve in parallel each side of the recursively division.
